...
创建班级 → 发布短诊断 → 查看班级证据 → 生成并审核动态分组草稿 → 确认下发干预 → 用明确标记的无提示迁移题复验。
...
- 学生必须完成当前任务中的该题后才能查看参考答案；个人练习或其他任务中的旧作答不能解锁当前任务。教师可查看题库答案。
- 提示请求由服务端写入审计记录，作答中的提示次数由服务端计算；客户端不能通过重置计数伪造“无提示”。
- “独立迁移”只统计干预或复测任务中被明确指定为迁移验证题、且无提示正确的作答。
