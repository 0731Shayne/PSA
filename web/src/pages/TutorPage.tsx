...
import { BookOutlined, BulbOutlined, DeleteOutlined, EyeOutlined, FileTextOutlined, PlusOutlined, RobotOutlined, SendOutlined, SettingOutlined, StopOutlined, UserOutlined } from "@ant-design/icons";
import { useNavigate, useSearchParams } from "react-router-dom";
import { apiClient } from "@/api/client";
import { MathMarkdown } from "@/components/MathMarkdown";
import { demoTutorAnswer, demoTutorSources, isDemoMode } from "@/demo/demoApi";

interface QuestionSource { kind?: "question"; ID: string; qtype: string; question: string; keypoint: string[]; hard_level: string }
interface MaterialSource { kind: "material"; id: string; document_id: string; document_name: string; content: string; similarity: number; positions?: unknown[]; backend?: "local" | "ragflow" }
type Source = QuestionSource | MaterialSource;
...
interface QuestionDetail extends QuestionSource { answer?: string; explanation?: string; can_reveal?: boolean; teacher_view?: boolean }
...
function modelLabel(model?: string) {
  ...
  if (model === "question-bank-fallback") return "题库与课程资料离线引导";
...
export default function TutorPage() {
  ...
  const sidebarContent = <>
      ...
      <div className="mt-5 flex min-h-0 flex-col border-t border-slate-200 pt-5 lg:flex-1">
        ...
        <div className="max-h-48 min-h-0 space-y-1 overflow-y-auto pr-1 lg:max-h-none lg:flex-1">
          {historyLoading && !sessions.length ? <div className="py-6 text-center"><Spin size="small" /></div> : historyError && !sessions.length ? <Alert type="error" showIcon message="历史会话加载失败" action={<Button size="small" onClick={refreshSessions}>重试</Button>} /> : sessions.length === 0 ? <p className="py-5 text-center text-sm text-slate-400">暂无历史会话</p> : sessions.map(item => <div key={item.id} className={`group flex items-center gap-1 rounded-xl px-1.5 transition ${activeSession === item.id ? "bg-teal-100 text-teal-900" : "text-slate-600 hover:bg-slate-100"}`}><button onClick={() => openSession(item.id)} className="min-w-0 flex-1 truncate rounded-lg px-2 py-2.5 text-left text-sm font-semibold" aria-current={activeSession === item.id ? "true" : undefined}>{item.title}</button><Popconfirm title="删除此会话？" okText="删除" cancelText="取消" onConfirm={() => deleteSession(item.id)}><Button aria-label={`删除会话：${item.title}`} type="text" danger size="small" icon={<DeleteOutlined />} /></Popconfirm></div>)}
        </div>
      </div>
  ...
  return <div className="mx-auto grid min-w-0 max-w-6xl gap-6 xl:grid-cols-[260px_minmax(0,1fr)]">
    ...
    <section className="flex h-[calc(100dvh-168px)] min-h-[560px] min-w-0 flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white lg:h-[calc(100vh-132px)] lg:min-h-[640px]">
      ...
      <div className="min-w-0 flex-1 overflow-y-auto p-4 sm:p-5 lg:p-8" role="log" aria-live="polite" aria-label="答疑消息">
        ...
        <div className="space-y-6">{messages.map((item, index) => <div key={item.id || index} className={`flex min-w-0 gap-2 sm:gap-3 ${item.role === "user" ? "justify-end" : "justify-start"}`}>{item.role === "assistant" && <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-teal-700 text-white" aria-hidden="true"><RobotOutlined /></span>}<div className={`min-w-0 max-w-[calc(100%-2.75rem)] rounded-2xl px-4 py-3.5 sm:max-w-[82%] sm:px-5 sm:py-4 ${item.role === "user" ? "bg-teal-700 text-white" : "bg-slate-50 text-slate-700"}`}>{item.role === "assistant" && !item.content ? <div className="flex items-center gap-2 py-1 text-sm text-slate-500"><Spin size="small" />正在检索题库与课程资料并组织引导…</div> : <div className="tutor-markdown min-w-0 text-[15px] leading-8"><MathMarkdown>{item.content}</MathMarkdown></div>}{item.sources && item.sources.length > 0 && <div className="mt-5 border-t border-slate-200 pt-4"><div className="mb-3 text-sm font-bold text-slate-600">回答依据 · 点击查看</div><div className="grid min-w-0 gap-2 sm:grid-cols-2">{item.sources.map((source, sourceIndex) => source.kind === "material" ? <button key={`material-${source.id}-${sourceIndex}`} onClick={() => setSelectedMaterial(source)} className="min-w-0 rounded-xl border border-sky-200 bg-white p-3 text-left transition hover:border-sky-400 hover:bg-sky-50"><div className="flex items-center justify-between gap-2"><span className="truncate text-sm font-extrabold text-sky-800"><FileTextOutlined className="mr-1" />课程资料</span>{source.similarity > 0 && <Tag color="blue" className="!mr-0">匹配 {Math.round(source.similarity * 100)}%</Tag>}</div><p className="mt-2 truncate text-sm font-semibold text-slate-700">{source.document_name}</p><p className="mt-1 line-clamp-2 text-sm leading-5 text-slate-500">{source.content}</p></button> : <button key={`question-${source.ID}`} onClick={() => openSource(source.ID)} className="min-w-0 rounded-xl border border-slate-200 bg-white p-3 text-left transition hover:border-teal-300 hover:bg-teal-50"><div className="flex items-center justify-between gap-2"><span className="truncate text-sm font-extrabold text-teal-800"><BookOutlined className="mr-1" />第 {sourceIndex + 1} 题 · {source.ID}</span><Tag color="cyan" className="!mr-0">{source.hard_level}</Tag></div><p className="mt-2 line-clamp-2 text-sm leading-5 text-slate-600">{source.question}</p></button>)}</div></div>}{item.model && <div className="mt-3 text-sm text-slate-500">回答来源：{modelLabel(item.model)}</div>}</div>{item.role === "user" && <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-700" aria-hidden="true"><UserOutlined /></span>}</div>)}</div>
        <div ref={bottom} />
      </div>
    ...
    <Drawer open={!!selectedMaterial} onClose={() => setSelectedMaterial(null)} width="min(680px, 100vw)" title={selectedMaterial?.document_name || "课程资料引用"}>{selectedMaterial && <div className="space-y-5"><Alert type="info" showIcon message="这是本次回答命中的课程资料片段" description="片段用于核对回答依据；完整文件的访问范围由教师配置。" /><div className="flex flex-wrap gap-2"><Tag color="blue">{selectedMaterial.backend === "local" ? "本地向量检索" : "RAGFlow 检索"}</Tag>{selectedMaterial.similarity > 0 && <Tag>综合匹配度 {Math.round(selectedMaterial.similarity * 100)}%</Tag>}</div><div className="question-markdown rounded-2xl border border-slate-200 bg-slate-50 p-5 text-slate-700"><MathMarkdown>{selectedMaterial.content}</MathMarkdown></div></div>}</Drawer>
  </div>;
}
