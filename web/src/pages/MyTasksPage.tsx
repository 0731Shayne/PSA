...
export default function MyTasksPage() {
  ...
  return <div className="course-page mx-auto max-w-6xl">
    <div className="page-heading flex flex-wrap items-end justify-between gap-4"><div><span className="page-index">STUDENT · ASSIGNMENTS</span><h1 className="text-2xl font-bold text-slate-950">我的任务</h1><p className="mt-1 max-w-3xl text-base leading-7 text-slate-600">完成教师发布的诊断与干预；最后一道题会检查你是否能在无提示下独立迁移。</p></div><Button icon={<ReloadOutlined />} onClick={load} loading={loading}>刷新任务</Button></div>
...
function TaskRow({ task, onOpen }: { task: Task; onOpen: () => void }) {
  ...
  return <article className="grid gap-4 px-5 py-4 md:grid-cols-[minmax(0,1fr)_180px_auto] md:items-center">
    ...
    <Button type={task.my_status === "completed" ? "default" : "primary"} icon={task.my_status === "completed" ? <CheckSquareOutlined /> : <ArrowRightOutlined />} disabled={!nextQuestion} onClick={onOpen}>{task.my_status === "completed" ? "查看任务" : done ? "继续完成" : "开始任务"}</Button>
  </article>;
}
