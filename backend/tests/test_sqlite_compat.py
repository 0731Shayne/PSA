    ...
    async with engine.begin() as connection:
        ...
        def schema(sync_connection):
            ...
            return (
                ...
                {column["name"] for column in inspector.get_columns("learning_assignments")},
        ...
        user_columns, assignment_columns, columns, indexes, teaching_columns, teaching_indexes = await connection.run_sync(schema)

    await engine.dispose()

    assert "assignment_id" in columns
    assert {"ocr_text", "ocr_provider", "ocr_status", "submitted_late"} <= columns
    assert {"session_version", "must_change_password", "is_active"} <= user_columns
    assert {"hint_policy", "transfer_question_id"} <= assignment_columns
